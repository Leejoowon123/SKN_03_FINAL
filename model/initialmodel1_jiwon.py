...
class MusicalRecommender:
    ...
    def load_and_preprocess_data(self):
        ...
        self.data = pd.read_csv('embedding.csv')  # Update the path to a relative one if necessary
    ...
    def prepare_training_data(self):
        ...
        X = pd.DataFrame({
            ...
            'percentage': numerical_data[:, 0],  # 첫 번째 수치형 변수
        })
    ...
    def create_deepfm_model(self):
        ...
        concatenated = Concatenate()([Flatten()(embeddings['title']), Flatten()(embeddings['cast']), Flatten()(embeddings['genre']), inputs['percentage']])
    ...
    def train_model(self):
        ...
        history = self.model.fit(
            ...
            epochs=20,
            verbose=1,
            validation_data=([X_test['title'], X_test['cast'], X_test['genre'], X_test['percentage']], y_test)
        )
        ...
        test_loss, test_acc = self.model.evaluate(
            [X_test['title'], X_test['cast'], X_test['genre'], X_test['percentage']],
            y_test, verbose=2
        )
...
# # Initialize the recommender system and train   #ipynb에서 실행하기 위에거 넣으면 됨
# recommender = MusicalRecommender()
# recommender.load_and_preprocess_data()
# test_loss, test_acc = recommender.train_model()  #초기모델 loss 0.59
